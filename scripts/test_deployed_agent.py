...
class DeployedCymbalBotTester:
    def __init__(self, agent_resource_name: str = None, config_file: str = None):
        """
        Initialize the tester with the deployed agent resource name.
        
        Args:
            agent_resource_name: The resource name of the deployed agent
            config_file: Path to the agent engine configuration file
        """
        if not agent_resource_name:
            ...
            if config_file and os.path.exists(config_file):
                with open(config_file, 'r') as f:
                    config = json.load(f)
                    agent_resource_name = config.get('agent_resource_name')
            else:
                # Fallback to default
...
async def main():
    ...
    config_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'config', 'agent_engine.json')
    
    try:
        # Create tester instance
        tester = DeployedCymbalBotTester(agent_resource_name, config_file)
        ...
        results_file = "test_results/deployed_test_results.json"
